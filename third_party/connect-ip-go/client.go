...
func NewClientConn(conn *http3.ClientConn) *ClientConn {
	return NewClientConnWithOptions(conn, "", nil, false)
}

// NewClientConnWithOptions creates a client connection with Cloudflare-compatible request options.
func NewClientConnWithOptions(conn *http3.ClientConn, protocol string, headers http.Header, ignoreExtendedConnect bool) *ClientConn {
	return &ClientConn{
		clientConn:            conn,
		requestProtocol:       protocol,
		additionalHeaders:     headers.Clone(),
		ignoreExtendedConnect: ignoreExtendedConnect,
	}
}
...
func (c *ClientConn) Dial(req *Request) (*Conn, *http.Response, error) {
	if req == nil {
		return nil, nil, errors.New("connect-ip: request is nil")
	}
	if c.requestProtocol != "" {
		req.req.Proto = c.requestProtocol
	}
	for k, values := range c.additionalHeaders {
		req.req.Header[k] = append([]string(nil), values...)
	}
...
func (c *ClientConn) dial(req *Request, closeConn func() error) (*Conn, *http.Response, error) {
	...
	if !c.ignoreExtendedConnect && !settings.EnableExtendedConnect {
