...
func newDatagramOnlyConn(str http3Stream) *Conn {
	return &Conn{
		...
		supportsControlCapsules: false,
		assignedAddressUpdates:  make(chan []AssignedAddress),
		addressRequests:         make(chan *addressRequestCapsule),
		availableRouteUpdates:   make(chan []IPRoute),
		dnsConfigurationUpdates: make(chan []DNSConfiguration),
		pref64Updates:           make(chan []netip.Prefix),
		closeChan:               make(chan struct{}),
	}
}
...
func (c *Conn) ReadPacket(b []byte) (n int, err error) {
	return c.readPacket(b, false)
}

// ReadPacketZeroCopy reads and validates an IP packet without copying it.
func (c *Conn) ReadPacketZeroCopy(allowAny bool) ([]byte, error) {
	data, err := c.receivePacket(allowAny)
	if err != nil {
		return nil, err
	}
	return data, nil
...
func (c *Conn) readPacket(b []byte, allowAny bool) (n int, err error) {
	packet, err := c.receivePacket(allowAny)
	if err != nil {
		return 0, err
	}
...
func (c *Conn) receivePacket(allowAny bool) ([]byte, error) {
start:
